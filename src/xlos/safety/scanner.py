    ...
    if isinstance(xspec, Mapping) and xspec.get("requires_admin") is True:
        return [
            Finding(
                ...
                "extensions.x_money_specific.requires_admin",
                "I.3",
            )
        ]
    return []
    ...
    if rules and (not isinstance(refusals, list) or not refusals):
        return [
            Finding(
                ...
                "extensions.constitution.hard_refusals",
                "III",
            )
        ]
    return []
...
def check_tax_disclaimer(m: Mapping[str, Any]) -> List[Finding]:
    """Article V.2 — finance agents that export tax data must declare not_tax_advice.

    Only fires when the agent's category is in the finance family AND the
    constitution rules reference tax operations explicitly.
    """
    if m.get("category") not in FINANCE_CATEGORIES:
        return []
    cons = _constitution(m)
    text = ""
    if isinstance(cons, Mapping):
    ...
    if not isinstance(disclaimers, Mapping) or disclaimers.get("not_tax_advice") is not True:
        return [
            Finding(
                "error",
                "V2-001",
                "x-money-tool agents referencing tax operations must set not_tax_advice=true",
                "extensions.x_money_specific.disclaimers.not_tax_advice",
                "V.2",
            )
        ]
    return []
    ...
    if prov.get("cite_sources") is False:
        findings.append(
            Finding(
                ...
                "extensions.provenance.cite_sources",
                "IV",
            )
        )
