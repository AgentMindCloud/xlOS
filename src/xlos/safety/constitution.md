...
1. **Schema** — `spec/v2.14/schema.json` makes the rules expressible (every Constitution clause maps to a manifest field).
2. **Scanner** — `xlos.safety.scanner` checks that each manifest satisfies these rules before it can be installed or distributed.
3. **Runtime** — agents must consult `xlos.safety.scanner` (or its embedded checks) at runtime before any consent-gated action.
...
2. **Ecosystem alignment.** Agents in the xlOS catalog should remain compatible with the wider Grok / X / xAI ecosystem and must not position themselves as adversaries to it. xlOS itself is cross-platform; the explicit "Built for xAI..." boilerplate that grok-agent required is stripped on ingest.
3. **Cross-platform.** xlOS is cross-platform (macOS, Linux, Windows). End-user instructions, launchers, and installation are Python-first and must run on all three operating systems without requiring PowerShell. Bash is allowed inside CI workflows on any GitHub runner image.
...
4. **No bypassing the safety scanner.** Agents must not include code that disables, monkey-patches, or skips `xlos.safety.scanner` checks.
...
This Constitution is enforced by `xlos.safety.scanner`, which runs in three places:
